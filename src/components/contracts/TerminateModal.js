...
export default function TerminateModal({ open, onClose, onSaved, contract }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      const data = await ContractService.update(contract.uuid, {
        status: 'terminated',
      ...
      if (data?.success) {
        ...
        setServerError(data?.message || 'Failed to terminate contract.');
      }
    } catch {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Terminate Contract" maxWidth="max-w-md">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {contract && (
          <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 space-y-1">
            <p className="text-xs text-amber-700">Contract</p>
            ...
            <div className="flex justify-between text-xs">
              ...
              <span className="font-medium text-gray-900">{contract.end_date || '—'}</span>
            </div>
          </div>
        )}
        ...
        <div className="flex items-center justify-end gap-3 pt-3 border-t border-gray-100">
          ...
          <button type="submit" className="btn-danger text-sm" disabled={saving}>
            {saving && <Spinner />}
            Terminate Contract
          </button>
        </div>
